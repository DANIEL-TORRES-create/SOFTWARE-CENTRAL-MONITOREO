  ...
  class ArdepeUI {
    ...
    layout() {
      ...
      return '<div class="shell ' + (driver ? 'driver-shell' : '') + '">' +
        '<header class="topbar"><img class="logo" src="' + logo + '" alt="ARDEPE SAC"><div class="title"><h1>' + (driver ? 'Mis atenciones' : 'Central Integral de Monitoreo') + '</h1><p>ARDEPE · Seguridad vial</p></div>'+(driver?'<button id="a-drive-back" class="drive-back" aria-label="Volver al panel de Geotab Drive" title="Volver a Drive">←</button>':'')+'<div class="identity"><span id="a-connection" class="connection">Preparando conexión</span><div id="a-identity" class="muted"></div></div></header>' +
    ...
    bind() {
      ...
      this.$('live').onclick = () => { this.mode = 'live'; this.$('history-banner').hidden = true; this.$('state').value = 'ALL'; if(this.role==='central'&&this.origin==='ALL')this.origin='GEOTAB';this.$('tabs').querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.dataset.origin===this.origin));this.refresh().catch(e => this.toast(e.message, true)); };
    ...
    async run(button, fn) { if (button.disabled) return; const html = button.innerHTML; button.disabled = true; button.innerHTML = '<span class="spinner"></span>Procesando…'; try { await fn(); } catch (error) { this.toast(error.message, true); } finally { if (button.isConnected) { button.disabled = false; button.innerHTML = html; } } }
    pause() { clearInterval(this.timer); clearInterval(this.clock); }
    focus() { this.stopBackgroundNotifications();this.resume(); }
    blur() { this.pause();if(this.role==='driver'&&this.service&&!this.demo)this.startBackgroundNotifications(this.service); }
    ...
    closeCentralCase() {
      this.selected=null;this.$('context').innerHTML='<div class="empty">Seleccione un caso para revisar su contexto.</div>';this.$('manager').textContent='';this.$('work').innerHTML='<div class="operation-start"><h3>Solicitud enviada</h3><p>El caso continúa en la bandeja con estado <b>Esperando respuesta del conductor</b>. Puede volver a abrirlo cuando lo necesite.</p></div>';this.renderList();
    }
    messagesHTML(c) {
    ...
    guide() {
      ...
      const topics=this.role==='driver'?
        ...
        [
          ...
          ['comunicar','Contactar al conductor','<ol><li>Elija Drive, teléfono, supervisor u otro canal.</li><li>Si usa Drive, escriba una solicitud clara y marque si requiere respuesta.</li><li>Las respuestas y evidencias aparecerán dentro del mismo caso.</li></ol>'],
          ...
          ['consulta','Histórico y PDF','<p>El Histórico contiene únicamente casos de esta implementación. Los eventos por gestionar se consultan en Geotab sin almacenarlos. El PDF se genera con la información actual y no se guarda en Drive.</p>']
        ];
