  ...
  class ArdepeUI {
    ...
    layout() {
      ...
      return '<div class="shell ' + (driver ? 'driver-shell' : '') + '">' +
        '<header class="topbar"><img class="logo" src="assets/logo.png" alt="ARDEPE SAC"><div class="title"><h1>' + (driver ? 'Mis atenciones' : 'Central Integral de Monitoreo') + '</h1><p>ARDEPE · Seguridad vial</p></div><div class="identity"><span id="a-connection" class="connection">Preparando conexión</span><div id="a-identity" class="muted"></div></div></header>' +
        ...
        '<div class="commandbar"><div class="operator"><label>Personal activo<select id="a-person"><option value="">Seleccione personal</option></select></label></div><button id="a-history">Histórico</button><button id="a-create" class="primary">' + (driver ? 'Reportar incidente' : 'Crear caso') + '</button><button id="a-recover">Recuperar envío</button><button id="a-guide">Guía</button>' + (!driver ? '<button id="a-admin" aria-label="Administración">⚙</button>' : '') + '</div>' +
        '<section class="metrics" id="a-metrics" aria-label="Resumen"></section>' +
        '<section class="filters"><nav class="tabs" id="a-tabs" aria-label="Origen">' + (driver ? [['ALL','Mis casos activos'],['NEW','Solicitudes nuevas'],['DONE','Finalizados']] : [['ALL','Todos activos'],['GEOTAB','Eventos Geotab'],['CENTRAL','Casos Central'],['CONDUCTOR','Reportes conductor']]).map(([id,label]) => '<button data-origin="' + id + '" class="' + (id === 'ALL' ? 'active' : '') + '">' + label + '</button>').join('') + '</nav><div class="filter-row"><select id="a-state" aria-label="Estado"><option value="ALL">Todos los estados activos</option><option value="OLD">Pendientes anteriores</option>' + Object.entries(D.STATES).map(([id,label]) => '<option value="' + id + '">' + label + '</option>').join('') + '</select><select id="a-priority" aria-label="Prioridad"><option value="ALL">Todas las prioridades</option>' + options(D.PRIORITIES) + '</select><input id="a-search" type="search" placeholder="Buscar placa, conductor o motivo" aria-label="Buscar casos"><button id="a-refresh">Actualizar</button></div></section>' +
        ...
        '<main class="workspace"><aside class="panel queue-panel"><header class="panel-heading"><h2 id="a-queue-title">Cola de atención</h2><small id="a-count">0 casos</small></header><div class="scroll" id="a-list"><div class="empty"><span class="spinner"></span>Consultando…</div></div><div class="pager"><button id="a-prev" aria-label="Página anterior">Anterior</button><span id="a-page"></span><button id="a-next">Siguiente</button></div></aside><section class="panel context-panel"><header class="panel-heading"><h2>Detalle del caso</h2><span>Información operativa</span></header><div id="a-context" class="scroll"><div class="empty">Seleccione un caso para revisar su contexto.</div></div></section><section class="panel management-panel"><header class="panel-heading"><h2>' + (driver ? 'Conversación con Monitoreo' : 'Atención y conversación') + '</h2><small id="a-manager"></small></header><div id="a-work" class="scroll"><div class="empty"><strong>Seleccione un evento o caso</strong>La atención, las respuestas y las evidencias se mostrarán aquí.</div></div></section></main></div>' +
    ...
    async select(item) {
      this.selected = item; this.renderList(); this.renderContext(); this.renderWork();
      if (item.version) {
    ...
    renderWork(preserve = false) {
      ...
      this.$('work').innerHTML = '<div class="detail-inline">' + this.contextHTML(c,true) + '</div><div class="content">' +
    ...
    guide() { const extra=this.service.settings&&(this.role==='driver'?this.service.settings.driverGuide:this.service.settings.centralGuide);this.dialog('Guía de '+(this.role==='driver'?'conductor':'atención'),'<div class="guide"><ol>'+ (this.role==='driver' ? '<li>Revise la solicitud y el valor detectado del evento.</li><li>Describa lo ocurrido y adjunte las evidencias disponibles. Cada mensaje queda registrado; las correcciones se agregan como mensajes nuevos.</li><li>Use Reportar incidente para informar un caso sin esperar una solicitud.</li><li>Consulte sus casos activos e historial. Una atención finalizada queda disponible para consulta.</li>' : '<li>Seleccione personal activo y revise el evento. Consultarlo no lo guarda en Sheets.</li><li>Inicie la atención para reservar y registrar el caso. Verifique el conductor antes del primer mensaje.</li><li>Solicite descargo por Drive o registre atención por teléfono, supervisor o validación interna.</li><li>Evalúe resultado y causa; registre acciones y seguimiento. También puede finalizar por Falsa alerta o No amerita descargo, documentando el motivo.</li><li>Registrar gestión guarda la etapa elegida. Solo Atención finalizada detiene el tiempo total.</li><li>El historial de atenciones reúne casos registrados. Eventos Geotab por gestionar busca eventos antiguos sin atención.</li>') + '</ol><div class="info">Paradas: HH:MM:SS. Velocidad: km/h. Maniobras: G. Los mensajes anteriores no se editan.</div>'+(extra?'<p>'+esc(extra)+'</p>':'')+'</div>'); }
